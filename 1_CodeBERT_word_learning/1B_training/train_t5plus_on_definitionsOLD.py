"""
This follows the codet5plus paper: https://arxiv.org/pdf/2305.07922
...
CodeT5+ 2B, 6B, 16B in which ... [not relevant, we use 220 and 770... ] ... . We employ the CodeT5
tokenizer and CodeGen tokenizer for these two groups of models respectively. In pretraining, we
adopt a stage-wise strategy to pretrain CodeT5+ first on the large-scale unimodal dataset and then on
the smaller bimodal dataset on a cluster with 16 A100-40G GPUs on Google Cloud Platform."

I plan to use the 220m and 770m models.

The small model follows the architecture described here (raffel et al 2020):
...
class DecoderOnlyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # TODO: gotta speed up training by concatenating all these words together in chunks of 512!
        ...
        input_ids = torch.tensor([tokenizer.pad_token_id] * self.max_length)  # Empty input_ids #THIS SEEMS WRONG BUG
